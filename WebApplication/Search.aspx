...
*/
    ...
    string SearchQuery
    {
        get
        {
            ...
            else
            {
                ...
                IStopper _Stopper2;
                string x = Request.QueryString[Preferences.QuerystringParameterName].ToString();
                string[] xx = x.Split(' ');
                if (Searcharoo.Engine.Search.flag == 1)
                {

                    int i = 1;// richTextBox1.Lines.Length;
                    Searcharoo.ArabicStopWords SW = new Searcharoo.ArabicStopWords();
                    for (int k = 0; k < i; k++)
                    {
                        string S = ""; string Temp = "";
                        Temp = x;//richTextBox1.Lines[k];
                ...
                else
                {
                    foreach (string word in xx)
                    {
                        ...
                        if (key.Length < 3)
                        {
                            _Stopper2 = new ShortStopper();
                            key = _Stopper2.StopWord(key);

                        }
                        else if (key.Length <= 4)
                        {
                            _Stopper2 = new ListStopper();
                            key = _Stopper2.StopWord(key);
                        }
                        else
                        {
                            _Stopper2 = new NoStopping();
                            key = _Stopper2.StopWord(key);
                        }
    ...
    public void Page_Load ()
    {
        ...
        foreach (char ss in SearchQuery)
        {
        ...
        else
        {
            ...
            Response.Redirect("~/WebApplication/result.txt");
    ...
    protected void SearchResults_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
...
</script><title>Search</title><meta http-equiv="robots" content="none"><style type="text/css">
		body {margin:10px 10px 10px 10px;background-color:white;}
		.heading {font-size:xx-large;font-weight:bold;color:white;filter:DropShadow (Color=#cccccc, OffX=5, OffY=5, Positive=true);}
		.copyright {font-size:xx-small;}
		body, td, a {font-family:trebuchet ms, verdana, arial, sans-serif;font-size:small;}
		.subheading {font-size:large;font-weight:bold;color:white;}
	</style></head><body><div class="main indexpage">
  <div class="header">
    <div class="header_resize">
      <div class="menu_nav">
        <ul>
          <li class="active"><a href="http://localhost:3359/login.aspx">Logout</a></li>
          <li><a href="http://localhost:3359/support.html"><span>Support</span></a></li>
          <li><a href="http://localhost:3359/about.html"><span>About Us</span></a></li>
          <li><a href="http://localhost:3359/blog.html"><span>Blog</span></a></li>
          <li><a href="http://localhost:3359contact.html"><span>Contact Us</span></a></li>
        </ul>
      </div>
      <div class="logo">
        <h1>&nbsp;</h1>
      ...
      <div class="clr"></div>
		...
		<asp:Repeater id="SearchResults" runat="server" OnItemCommand="SearchResults_ItemCommand">
		    <HeaderTemplate>
			<font color="lightgreen">    <p><%=_NumberOfMatches%> results for <%=_Matches%> took <%=_DisplayTime%></p>
		    </HeaderTemplate>
		    <ItemTemplate>
			...
			<font color="black">    <a href="<%# DataBinder.Eval(Container.DataItem, "Url") %>" target="_blank" title="open in new window" style="font-size:x-small">&uarr;</a>
			    <font color="black">(<%# DataBinder.Eval(Container.DataItem, "Rank") %>)</font>
			    <br><%# DataBinder.Eval(Container.DataItem, "Description") %>...
			    <br><font color="black"><%# DataBinder.Eval(Container.DataItem, "Url") %> - <%# DataBinder.Eval(Container.DataItem, "Size") %>
			    bytes</font>
			    <font color="black">- <%# DataBinder.Eval(Container.DataItem, "CrawledDate") %></font><p>
		    </ItemTemplate>
		    <FooterTemplate>
